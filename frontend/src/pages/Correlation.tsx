...
export function Correlation() {
  ...
  const compute = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : copy.pages.correlation.failed);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold">{copy.pages.correlation.title}</h1>
      </div>

      {/* Controls */}
      <div className="flex flex-col gap-4 border rounded-lg p-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium">{copy.pages.correlation.assetCodes}</label>
          <input
            type="text"
            value={codes}
          ...
          <p className="text-xs text-muted-foreground">
            {copy.pages.correlation.assetCodesHint}
          </p>
        </div>

        <div className="flex flex-wrap gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">{copy.pages.correlation.windowDays}</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">{copy.pages.correlation.method}</label>
            <div className="flex gap-1.5">
        ...
        >
          {loading ? copy.common.loading : copy.pages.correlation.compute}
        </button>
      </div>
