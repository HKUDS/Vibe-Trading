...
export function ChanTrainingReviews() {
  ...
  const remove = async (item: ChanTrainingSession) => {
    if (!window.confirm(`确定删除这次${item.status === "active" ? "未结束的训练" : "复盘记录"}吗？删除后不可恢复。`)) return;
