...
- **Korea equities (KRX: KOSPI/KOSDAQ)** via pykrx using `<CODE>.KS` (KOSPI, e.g. `005930.KS`) or `<CODE>.KQ` (KOSDAQ, e.g. `247540.KQ`) — free, no API key (`pip install "vibe-trading-ai[krx]"`; yahoo/yfinance fallback needs no extra). The `KoreaEquityEngine` models same-day round trips (no T+1), the ±30% daily price limit, sell-only transaction tax, 1-share lots, and no shorting by default (set `allow_short` to opt in).
