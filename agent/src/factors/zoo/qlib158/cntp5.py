...
__alpha_meta__ = {
    ...
    'decay_horizon': 5,
    'min_warmup_bars': 5,
    'notes': 'A missing close leaves window+1 rolling rows NaN; on sparse panels this can trigger the >95% NaN registry guard (from ~~40% missing bars).',
}
...
def compute(panel: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    c = panel['close']
    prev = c.shift(1)
    up = (c > prev).astype('float64').where(c.notna() & prev.notna())  # a missing close is not a down day
