...
class GroundingLedger:
        ...
        for index, line in enumerate(lines):
            ...
            for segment in _split_clauses(line):
                ...
                for value, value_kind in GroundingLedger._measure_numbers_with_kinds(
                    ...
                    if value_kind == "tail_risk":
                        measure_, confidence_, cursor = _tail_risk_identity_at(
                            segment, value, cursor
                        )
                        observed = self._analysis_value_observed(
                            value, value_kind, (measure_, confidence_)
                        )
                    else:
                        observed = self._analysis_value_observed(value, value_kind)
                    if not observed:
                        unsupported.append((value, value_kind))
                if not unsupported:
                    continue
    ...
    def _measure_numbers_with_kinds(
        ...
        masked = _SHORT_DATE_RE.sub(" ", masked)
        masked = _DASH_DATE_RE.sub(" ", masked)
        # The confidence in a tail-risk frame ("VaR 95%: -1.57%") is the
        # claim's identity, never its measurement — without the mask the 95%
        # itself gets measured and rejected against tail-risk evidence (#1427).
        masked = _TAIL_RISK_CONFIDENCE_RE.sub(" ", masked)
