...
class TaskStore:
    ...
    def load_all(self) -> list[SwarmTask]:
        ...
        for path in sorted(self._tasks_dir.glob("task-*.json")):
            tasks.append(SwarmTask.model_validate_json(path.read_text(encoding="utf-8")))
        return tasks

    def update_status(self, task_id: str, status: TaskStatus, **kwargs: str | int | list[str] | None) -> SwarmTask:
...
def topological_layers(tasks: list[SwarmTask]) -> list[list[str]]:
    ...
    if processed != len(tasks):
        raise ValueError(f"DAG contains a cycle: processed {processed}/{len(tasks)} tasks")
