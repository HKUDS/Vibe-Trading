...
> Link convention: every link to `references/` in this document is written with the **skill-name prefix** (for example `references/...`). The `read_file` tool resolves paths rooted at `skills/`, so omitting the prefix makes the read fail. When adding new skill docs, always use the `<skill-name>/references/...` form.
