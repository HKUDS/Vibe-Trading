"""API authentication and loopback security helpers."""
...
def _api_server_module():
    """Return the host ``api_server`` module when it is already imported."""
    return sys.modules.get("api_server")


def _host_attr(name: str, fallback):
    """Read a compatibility attribute from ``api_server`` when present."""
    host = _api_server_module()
...
def _is_allowed_loopback_host(host: str) -> bool:
    ...
    extra_hosts = _host_attr("_EXTRA_LOOPBACK_HOSTS", _EXTRA_LOOPBACK_HOSTS)
    return normalized in _DEFAULT_LOOPBACK_HOSTS or normalized in extra_hosts
...
def _trusted_docker_loopback_ip(ip: ipaddress._BaseAddress) -> bool:
    ...
    if not _env_flag_enabled(_DOCKER_LOOPBACK_ENV):
        return False
    gateway_reader = _host_attr("_default_gateway_ips", _default_gateway_ips)
    return ip in gateway_reader()
