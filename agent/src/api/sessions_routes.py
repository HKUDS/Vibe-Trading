...
def register_sessions_routes(app: FastAPI) -> None:
    ...
    def _host_is_local_client(request: Request) -> bool:
        h = _sys.modules.get("api_server") or _sys.modules.get("agent.api_server")
        return h._is_local_client(request)
    ...
    async def send_message(session_id: str, payload: SendMessageRequest, http_request: Request):
        ...
        try:
            result = await svc.send_message(
                ...
                governance_surface="local_api" if _host_is_local_client(http_request) else "remote_api",
            )
            return result
        except ValueError as exc:
