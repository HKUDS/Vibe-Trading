"""FX-layer contract: ISO validity is checked apart from rate availability."""
...
def test_build_rates_anchors_on_usd():
    assert RATES == {"USD": Decimal("1"), "CNY": Decimal("7"), "HKD": Decimal("8")}


def test_to_usd_converts_and_names_the_gap():
...
def test_value_position_without_rate_fails_closed_instead_of_pricing_as_usd():
    with pytest.raises(PortfolioContractError, match="KRW"):
        value_position(_krw_row(), rates=RATES)


def test_value_position_converts_when_the_rate_exists():
...
def test_account_total_converts_a_valid_iso_currency_when_rated():
    account = {"account": {"currency": "EUR", "total_equity": "920"}}
    total = account_total_usd("sample", account, {**RATES, "EUR": Decimal("0.92")})
    assert total == Decimal("1000")


def test_parse_settings_accepts_any_valid_iso_display_currency():
    settings = parse_settings({"display_currency": "eur", "sources": []})
    assert settings.display_currency == "EUR"


def test_parse_settings_rejects_a_non_iso_display_currency():
