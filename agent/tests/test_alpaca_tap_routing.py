...
def test_denied_order_is_blocked(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        al.tap_forward,
        "forward",
        lambda *a, **k: {"ok": False, "decision": "denied", "status": None, "body": None, "error": "denied"},
    )

    result = al.place_order(_paper_cfg(), symbol="TSLA", side="buy", notional=10000, order_type="market")
...
def test_client_order_id_is_deterministic_for_idempotency(monkeypatch) -> None:
    ...
    def fake_forward(target, method, body, cred_headers, **_):
        ...
        return {
            ...
            "decision": "forwarded",
            "status": 200,
            "body": json.dumps({"id": "ord", "status": "new"}),
            "error": None,
        }
    ...
    kw = dict(symbol="AAPL", side="buy", quantity=1, order_type="limit", limit_price=1, time_in_force="day")
...
def test_tap_credential_name_is_overridable(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        al.tap_forward,
        "forward",
        lambda target, method, body, cred_headers, **_: (
            captured.update(cred_headers=dict(cred_headers))
            or {
                "ok": True,
                "decision": "forwarded",
                "status": 200,
                "body": json.dumps({"id": "x", "status": "new"}),
                "error": None,
            }
        ),
    )
...
def test_read_fails_closed_when_tap_errors(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        al.tap_forward,
        "forward",
        lambda *a, **k: {"ok": False, "decision": "error", "status": 403, "body": None, "error": "host not allowed"},
    )
...
def test_denied_cancel_is_blocked(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        al.tap_forward,
        "forward",
        lambda *a, **k: {"ok": False, "decision": "denied", "status": None, "body": None, "error": None},
    )
...
def test_live_mandate_deny_blocks_before_any_tap_call(monkeypatch) -> None:
    calls: list = []
    monkeypatch.setattr(al.tap_forward, "tap_enabled", lambda: True)
    monkeypatch.setattr(al.tap_forward, "forward", lambda *a, **k: calls.append(a) or {"ok": False})
    ...
    out = gate.execute_live_order(
        ...
        place_kwargs={"symbol": "AAPL", "side": "buy", "notional": 500.0, "order_type": "market"},
    )
