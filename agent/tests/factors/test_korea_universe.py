...
def _korea_panel(n_rows: int = 40, symbols=("005930.KS", "247540.KQ")) -> dict:
    ...
    cols = list(symbols)
    close = pd.DataFrame(100.0 + np.cumsum(rng.randn(n_rows, len(cols)), axis=0), index=idx, columns=cols).abs() + 1.0
...
def test_representative_factor_computes_on_korea_panel(registry: Registry, alpha_id: str) -> None:
    listed = set(registry.list(universe="equity_kr"))
