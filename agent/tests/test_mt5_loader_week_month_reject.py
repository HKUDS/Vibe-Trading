...
def test_unsupported_interval_returns_empty_without_daily_rewrite() -> None:
    ...
    with patch.object(DataLoader, "_fetch_one") as mock_fetch:
        out = DataLoader().fetch(["EURUSD"], "2024-01-01", "2024-01-31", interval="2H")
    assert out == {}
...
def test_week_interval_uses_w1_not_daily() -> None:
    frame = MagicMock()
    frame.empty = False
    with patch("backtest.loaders.mt5_loader.cached_loader_fetch", return_value=frame) as mock_cache:
        out = DataLoader().fetch(["EURUSD"], "2024-01-01", "2024-01-31", interval="1W")
