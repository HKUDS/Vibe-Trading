...
Vibe-Trading は MCP-compatible client 向けに 61 MCP tools を公開します。stdio subprocess として動作し、server setup は不要です。Core research tools は HK/US/crypto で API key なしに動作し、trading connector tools は選択中の connector profile を使います。LLM key が必要なのは `run_swarm` のみです。
...
│   ├── mcp_server.py               # MCP サーバー — OpenClaw / Claude Desktop 向け 61 tools
│   │
│   ├── src/
