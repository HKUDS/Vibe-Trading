...
> **支持的 LLM providers：** OpenRouter、Requesty、AnyRouter.top（区域 Responses API）、OpenAI、DeepSeek、Gemini、Groq、DashScope/Qwen、Zhipu、Moonshot/Kimi、MiniMax、Xiaomi MIMO、Z.ai、Ollama（本地）。配置见 `.env.example`。
...
对于 AnyRouter.top 区域网关中必须走 Responses API 的模型，请使用专用 provider，
而不是 OpenAI-compatible Chat Completions 适配器：
